...
import DashboardTwoAlertCache from './js/DashboardTwoAlertCache.js';
...
export class StockTableTwoAlert extends React.Component {
    ...
    initialAlert(callback) {
        ...
        this.autoInterval = setInterval(() => {
            ...
            if (this.state.continueAnimation) {
                ...
                let length = 0;
                while (index < 897) {
                    const cache = DashboardTwoAlertCache.get(index);
                    let inRange = 0;
    ...
    async _addToNotificationsMenu(index) {
        const stock = DashboardTwoCache.get(index).StockCode;
        const currentPrice_state = parseInt(DashboardTwoCache.get(index).ChangeArray[0]);
        const currentPrice = parseInt(DashboardTwoCache.get(index).CurrentPrice);
        const previousPrice = parseInt(DashboardTwoCache.getPreviousPrice(index));
